...
\textbf{可信賴專業活動} - 心臟內科醫師在臨床實務中能夠穩立勝任的專業工作單元
\end{block}
...
\item 急慢性心臟病處理
\item 心臟檢查技術
\item \textcolor{emergency_red}{侵入性處置操作}
\item 病患照護協調
\end{itemize}
...
% 標示重點
\node[above] at (2,1.2) {\scriptsize 需監導};
\node[above] at (6,1.2) {\scriptsize 穩立作業};
...
\vspace{0.5cm}
\begin{itemize}
\item \textbf{Level 1-2}：在監導下學習與執行
\item \textbf{Level 3}：能穩立勝任標準作業
...
EPA 11 & \textcolor{emergency_red}{\textbf{CCU侵入性處置}} & \textcolor{emergency_red}{\textbf{插管、動脈導管、穿刺術}} \\
\hline
\end{tabular}
\end{table}

\vspace{0.3cm}
\begin{alertblock}{新增重點}
\textbf{EPA 11}涵蓋CCU重要的侵入性處置技術，包括氣管插管、動脈導管設置、心包膜穿刺術、胸腔穿刺術等關鍵技能。
\end{alertblock}
\end{frame}
...
能夠穩立進行心血管疾病相關的完整病史詢問，包括症狀評估、危險因子識別、家族史調查，並能初步評估心血管風險。
\end{alertblock}
...
\begin{alertblock}{核心描述}
能夠穩立判讀12導程心電圖，識別正常與異常變化，包括心律不整、ST-T變化、傳導異常等，並提出臨床相關性分析。
\end{alertblock}
...
\begin{frame}{EPA 11: CCU侵入性處置操作與管理}
\begin{alertblock}{核心描述}
能夠在心臟加護病房環境下，安全且熟練地執行各種侵入性處置，包括氣管內管插管、動脈導管設置、心包膜穿刺術、胸腔穿刺術，並能評估適應症、預防併發症、提供後續監測與照護。
\end{alertblock}
...
\hline
\rowcolor{emergency_blue!20}
\textbf{處置項目} & \textbf{Level 3 能力要求} & \textbf{Level 5 專家能力} \\
\hline
氣管插管 & 評估困難插管風險、執行快速序列插管 & 處理極困難插管、纖維支氣管鏡插管 \\
\hline
動脈導管 & 橈動脈與股動脈穿刺、波形判讀校正 & 困難穿刺、超音波導引、併發症處理 \\
\hline
心包穿刺 & 超音波評估、執行穿刺引流 & 複雜心包疾病、心包切開術 \\
\hline
胸腔穿刺 & X光超音波判讀、安全穿刺點選擇 & 影像導引穿刺、胸腔鏡技術 \\
\hline
\end{tabular}
\end{table}

\vspace{0.3cm}
\begin{alertblock}{安全品質指標}
• 插管成功率 >95\% • 動脈導管感染率 <2\% • 心包穿刺併發症 <5\% • 胸腔穿刺氣胸率 <3\%
\end{alertblock}
\end{frame}
...
\hline
技能操作類 & DOPS & 直接觀察評估 \\
(EPA 2, 4, \textcolor{emergency_red}{11}) & \scriptsize Direct Observation & 同儕回饋 \\
\hline
臨床推理類 & Mini-CEX & 病例討論 \\
...
\item \textcolor{emergency_red}{\textbf{侵入性處置}}：每次執行時評估
\end{itemize}
\end{block}
\end{frame}
...
% 月份標記
\foreach \x/\month in {0/第1月, 2.4/第2月, 4.8/第3月, 7.2/第4月, 9.6/第5月, 12/第6月} {
  \draw[emergency_blue] (\x,-0.2) -- (\x,0.2);
  \node[below, emergency_blue] at (\x,-0.3) {\scriptsize \month};
}

% EPAs安排
\node[above, align=center] at (1.2,0.8) {\scriptsize EPA 1,2,3\\基礎評估};
\node[above, align=center] at (3.6,0.8) {\scriptsize EPA 4,5,6\\診斷急症};
\node[above, align=center] at (6,0.8) {\scriptsize EPA 7,8,10\\慢病溝通};
\node[above, align=center] at (8.4,0.8) {\scriptsize EPA 9\\整合評量};
\node[above, align=center, text=emergency_red] at (10.8,0.8) {\scriptsize \textbf{EPA 11}\\CCU處置};

% 連接線
\foreach \x in {1.2, 3.6, 6, 8.4, 10.8} {
...
EPA 11納入CCU輪訓期間重點評量，確保侵入性處置技能的安全性與熟練度。每月選擇特定EPAs進行深度評量，確保學習者能循序漸進建立專業能力。
\end{alertblock}
\end{frame}
...
% 特別標記EPA 11
\node[above, align=center, text=emergency_red] at (1.5,1.8) {\scriptsize \textbf{EPA 11}\\CCU訓練設計};
\end{tikzpicture}
\end{center}
...
\begin{block}{關鍵里程碑}
\begin{itemize}
\item \textbf{3個月}：完成EPA設計與教師訓練（含EPA 11特殊訓練）
...
\item 心導管影像庫
\item 心電圖題庫
\item \textcolor{emergency_red}{CCU模擬訓練室}
\end{itemize}
...
\item 跨科際交流
\item 國際經驗分享
\item \textcolor{emergency_red}{急重症專家指導}
\end{itemize}
...
\item 定期校正會議
\item 外部品質稽核
\item \textcolor{emergency_red}{安全監控機制}
\end{itemize}
\end{columns}
\end{frame}
...
\item 個別化學習支持
\item 能力認證更客觀
\item \textcolor{emergency_red}{侵入性技能標準化}
\end{itemize}
\end{block}
...
\item 醫療品質提升
\item 團隊協作強化
\item \textcolor{emergency_red}{CCU處置安全性提升}
\end{itemize}
\end{block}

\begin{alertblock}{長期影響}
建立以能力為導向的醫學教育文化，培養更優秀的心臟內科專科醫師，特別強化急重症處置能力。
\end{alertblock}
\end{frame}
...
% 特別標示EPA 11
\node[below, text=emergency_red, font=\footnotesize] at (0,-3) {特別注重EPA 11的安全訓練與設備投資};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}{討論議題}
\begin{enumerate}
\item 現有教學資源是否足夠支持EPAs實施（特別是EPA 11）？
\item 評量者訓練的具體規劃與時程安排？
\item 與現行住院醫師評量制度如何整合？
\item 學習者接受度與適應性評估？
\item 跨科EPAs的協調與統一標準？
\item 國際認證與接軌的考量？
\item \textcolor{emergency_red}{CCU侵入性處置的安全監控機制？}
\item \textcolor{emergency_red}{急重症技能的標準化評估方式？}
\end{enumerate}
...
\begin{itemize}
\item 完成11個心臟內科EPAs的標準化設計
\item 建立完整的評量工具與流程
\item 訓練足夠的合格評量者
\item 建置數位化學習與評量平台
\item \textcolor{emergency_red}{建立CCU模擬訓練設施}
\end{itemize}
\end{alertblock}
...
\item 發展創新教學方法
\item 進行教學成效研究
\item \textcolor{emergency_red}{建立侵入性處置認證制度}
\end{itemize}
\end{block}
...
\item 推廣至其他醫學專科
\item 建立持續改善機制
\item \textcolor{emergency_red}{成為急重症處置教育典範}
\end{itemize}
\end{exampleblock}
\end{frame}
...
\item 從單向教學到互動發展
\item 從孤立評估到整合成長
\item \textcolor{emergency_red}{從基礎訓練到急重症專精}
\end{itemize}
...
\vspace{0.5cm}
\textcolor{emergency_red}{\scriptsize 新增EPA 11: CCU侵入性處置操作與管理}
\end{center}
\end{frame}
