...
\begin{frame}{本院同仁於AMEE 2025發表成果}
\frametitle{本院同仁於AMEE 2025發表成果}
\vspace{-0.3cm}

\begin{table}[h]
\centering
\fontsize{6pt}{7.5pt}\selectfont
\begin{tabular}{p{1.3cm}p{1.8cm}p{5.2cm}p{2.2cm}}
\toprule
\textbf{發表者} & \textbf{單位職類} & \textbf{發表主題} & \textbf{發表形式} \\
\midrule
廖士程 & 精神醫學部醫師 & Innovative Integration of Resilience Training in Medical Education: Addressing Difficult Doctor-Patient Encounters & Short Communication \\
\midrule
黃沅羚 & 急診醫學部醫師 & Video-Based Assessment as an Alternative to On-Site Evaluation Using the Team Emergency Assessment Measure (TEAM) Questionnaire: An In-Situ Simulation Study & e-Poster \\
\midrule
張家豪 & 急診醫學部醫師 & Enhancing In Situ Simulation as a Tool for Interprofessional Continuing Professional Development & e-Poster (In Person) \\
\midrule
陳湘欣 & 護理部護理師 & Empowering Emergency Confidence through Technology: A Study on the Effectiveness of 3D Vista VR Training for New Emergency Nurses & Short Communication \\
\midrule
黃詩婷 & 護理部護理師 & Innovative Teaching of Digital Situation Escape Game for Nursing Assessment & Short Communication (Online) \\
\midrule
劉珮芳 & 藥劑部藥師 & Mastering PharmGKB: A Multimodal Learning Approach to Precision Medicine for Busy Pharmacists & e-Poster \\
\midrule
于心怡 & 檢驗醫學部檢驗師 & Healing and Empathy: Exploring Innovative Approaches to Holistic Medical Education Through Film Appreciation & e-Poster \\
\midrule
侯采辰 & 教學部行政 & Achieving Comparable Competency Through Structured POCUS Training: Insights from an EPA-Based Evaluation & Short Communication \\
\midrule
廖佳怡 & 教學部行政 & Improving quality of medical education by innovative teaching contest & Short Communication (Online) \\
\midrule
黃國良 & 教學部行政 & From preparation to performance: A comparative analysis of team success on evidence-based medicine contest & e-Poster \\
\bottomrule
\end{tabular}
\end{table}
\vspace{0.2cm}
\textbf{註：}本表僅列出AMEE會議發表成果，另有台灣醫學教育年會發表1篇。
\end{frame}

\begin{frame}{本院同仁於台灣醫學教育年會發表成果}
\frametitle{本院同仁於台灣醫學教育年會發表成果}

\begin{table}[h]
\centering
\tablesize
\begin{tabular}{p{1.5cm}p{2.5cm}p{6cm}p{1.5cm}}
\toprule
\textbf{發表者} & \textbf{單位職類} & \textbf{發表主題} & \textbf{發表形式} \\
\midrule
曾紀萍 & 復健部物理治療師 & Enhancing Physical Therapy Trainee Assessment: A Comparison of New and Existing Entrustable Professional Activities Forms & Poster \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{block}{本院同仁發表成果總結}
\begin{itemize}
    \item \textbf{AMEE 2025}：共10篇發表
    \begin{itemize}
        \item Short Communication：5篇（3篇現場、2篇線上）
        \item e-Poster：5篇（4篇線上、1篇現場）
    \end{itemize}
    \item \textbf{台灣醫學教育年會}：共1篇發表
    \item \textbf{跨領域參與}：涵蓋醫師、護理師、藥師、檢驗師、物理治療師及行政人員
\end{itemize}
\end{block}
\end{frame}
