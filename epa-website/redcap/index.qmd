---
title: "REDCap EPA Survey 發送流程"
subtitle: "REDCap Survey Distribution Workflow"
---

::: {.callout-note}
## 適用情境
多位評核醫師 × 多位被評核學員（多對多）。一筆 record = 一個「評核醫師 × 被評核學員」配對。
:::
...
| `evaluated_name` | 被評核學員姓名 | 李小芬 |
| `evaluated_id` | 學員學號/員工編號 | N001234 |
| `form_name` | 要填的次專科 form | `cv_epa`、`chest_epa` … |

::: {.callout-important}
一位醫師評 N 位學員 → 該醫師的 email 出現 N 次，對應不同學員。
:::
...
3. 建議依 `{prefix}_evaluated_name` 分組，計算各學員的 EPA 平均等級
...
| 次專科 | Form Name | Email 欄位 | 科別預設值 |
|--------|-----------|------------|-----------|
| 胸腔內科 | `chest_epa` | `chest_assessor_email` | 胸腔內科 |
| 心臟內科 | `cv_epa` | `cv_assessor_email` | 心臟內科 |
| 腸胃內科 | `gi_epa` | `gi_assessor_email` | 腸胃內科 |
| 腎臟內科 | `nephro_epa` | `nephro_assessor_email` | 腎臟內科 |
| 內分泌科 | `endocrine_epa` | `endocrine_assessor_email` | 內分泌科 |
| 血液科 | `hema_epa` | `hema_assessor_email` | 血液科 |
| 感染科 | `id_epa` | `id_assessor_email` | 感染科 |
| 腫瘤科 | `onco_epa` | `onco_assessor_email` | 腫瘤科 |
| 風濕免疫科 | `rheuma_epa` | `rheuma_assessor_email` | 風濕免疫科 |
| 神經科 | `neuro_epa` | `neuro_assessor_email` | 神經科 |
...
::: {.callout-warning}
- **一筆 record = 一個「評核醫師 × 被評核學員」配對**，不可混用
